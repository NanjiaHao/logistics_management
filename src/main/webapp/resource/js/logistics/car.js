$(function () {
    $('#ermPayFilmListTable thead th .checker').removeClass('checker');
    window.ermPayFilmListTable = $('#ermPayFilmListTable').DataTable({
        "processing": true,
        "serverSide": true,
        "searching": false,
        "ordering": false,
        ...
        "ajax": {
            ...
            "type": "POST",
            "data": function (data) {
                $($("#ermPayFilmListForm").serializeArray()).each(function (i, obj) {
                    if (obj.value) {
        ...
        "columns":[
            ...
            {
                "data": function (data) {
                    return ermPayFilmCellData(data, "vehicleCode");
                }
            },
            {
                "data": function (data) {
                    return ermPayFilmCellData(data, "employeeName");
                }
            },
            {
                "data": function (data) {
                    return ermPayFilmCellData(data, "status");
                }
            }
        ],
...
function ermPayFilmCellData(data, cell) {
    ...
    switch (cell) {
        case "checkbox":
            ...
            break;
        case "status":
        	_html = '<td><a onclick="doView(this)">'+data.paymentNumber+'</a></td>'+
                    '<input type="hidden" name="paymentHeaderId" value="'+data.paymentHeaderId+'"/>'
            break;
    }
